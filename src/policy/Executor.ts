...
export async function executePolicies(plugins: any, orchestratorOutStore: N3.Store, logger: Logger) {

  const mainSubject = fetchMainSubject(orchestratorOutStore, logger);
  const origin = fetchOrigin(orchestratorOutStore, logger);
  const policies = await extractPolicies(orchestratorOutStore, "none", {}, logger);
  ...
  for (const policy of Object.values(policies)) {
    ...
    const policyStore = extractGraph(orchestratorOutStore, idNode);
    const mainStore = extractGraph(orchestratorOutStore, mainSubject);
